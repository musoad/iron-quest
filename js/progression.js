(() => {
  ...
  const api = {
    ...
    getWeekNumberFor: (iso) => getWeekNumber(iso),
    levelFromTotalXp,
    starsForDay
  };

  window.IronQuestProgression = api;

  // Backward compat (deine alten Scripts nutzen das teilweise)
  window.Progression = {
    weekNumberFor: (iso) => api.getWeekNumberFor(iso),
  };
})();
