(() => {
  ...
  async function renderChallenges(el){
    ...
    const loot = (window.IronQuestLoot && window.IronQuestLoot.getState ? window.IronQuestLoot.getState() : (window.IronQuestLoot && window.IronQuestLoot.load ? window.IronQuestLoot.load() : { chests:0, inv:[] }));
    ...
    el.innerHTML = `
      ...
      <div class="row2">
        <div class="card">
          ...
          <div class="pill"><b>Reward:</b> +${(dailyDef && dailyDef.reward)||0} XP + 1 Chest</div>
          <div class="btnRow">
        ...
        <div class="card">
          ...
          <div class="pill"><b>Reward:</b> +${(weeklyDef && weeklyDef.reward)||0} XP + 2 Chests</div>
          <div class="btnRow">
    ...
    el.querySelector("#qd").onclick = async ()=>{
      ...
      if(!ok) (window.Toast && window.Toast.toast)("Daily", "Noch nicht erfüllt oder bereits geclaimed.");
      await renderChallenges(el);
    };
    el.querySelector("#qw").onclick = async ()=>{
      const ok = await window.IronQuestRPG.claimWeekly();
      if(!ok) (window.Toast && window.Toast.toast)("Weekly", "Noch nicht erfüllt oder bereits geclaimed.");
      await renderChallenges(el);
    };
    el.querySelector("#qs").onclick = async ()=>{
      const ok = await window.IronQuestRPG.claimStory();
      if(!ok) (window.Toast && window.Toast.toast)("Story", "Noch nicht bereit oder bereits geclaimed.");
      await renderChallenges(el);
    };

    el.querySelector("#openChest").onclick = ()=>{
      const api = window.IronQuestLoot;
      const res = (api && api.rollDrop) ? api.rollDrop() : ((api && api.roll) ? api.roll() : { ok:false });
      if (!res || !res.ok) return (window.Toast && window.Toast.toast)("Chest", "Keine Chest verfügbar.");

      // Support multiple loot response shapes
      const msg = res.drop || res.msg || ((res.item && res.item.name) ? `Obtained: ${res.item.name}` : "Nothing (XP dust)");
      (window.Toast && window.Toast.toast)("Chest opened", msg);
    ...
    invUl.innerHTML = "";
    if (!(loot.inv && loot.inv.length)) invUl.innerHTML = "<li>—</li>";
    else{
    ...
    window.IronQuestRPG.ACHIEVEMENTS.forEach(a=>{
      const done = !!(state.ach && state.ach[a.id] && state.ach[a.id].done);
      let progText="";
      if (!a.type){
